...
## ソリューションファイルの作成
.\CreateDummyFiles\CreateDummyFiles.slnx
```xml
<Solution>
  <Project Path="CreateDummyFiles/CreateDummyFiles.csproj" />
  <Project Path="TestProject1/TestProject1.csproj" />
</Solution>
```

## プロジェクトファイルの修正
.\CreateDummyFiles\CreateDummyFiles\CreateDummyFiles.csproj
```xml
...
# ビルド
dotnet build .\CreateDummyFiles\CreateDummyFiles.slnx -c Release -p:InvariantGlobalization=false
dotnet build .\CreateDummyFiles\TestProject1\TestProject1.csproj
# 単体テスト
dotnet test .\CreateDummyFiles\TestProject1\TestProject1.csproj
# Usage
CreateDummyFiles\CreateDummyFiles\bin\Release\net10.0\CreateDummyFiles.exe -h
```
